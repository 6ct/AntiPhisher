...
var TRUE = true, FALSE = false, NULL = null, undefined = void[];
...
(function() {
	...
	while (idx--) {
		...
		if (global[globalObject] !== undefined) {
			if (!global[globalObject].name) {
				global[globalObject].name = globalObject;
			}
		}
	}
}());
...
var _getConfigurationType = function(val) {
	...
	if (!val.name) {
		if (val === global[sElement]) { return _types[sElement]; } // Firefox doesn't allow setting the name of Element
...
var o = {
	...
	any: function(...args) {
		var args = _convertConfigurationTypes(args);
...
var api = function() {
	var overload = function overload(...args) {
		return overload._call(overload, args);
	};
